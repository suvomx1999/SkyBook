    ...
    const info = await transporter.sendMail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <p>Hi ${user.name},</p>
          <p>Your booking has been cancelled. Here are the details for your records:</p>
          ...
          <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            <hr style="border: 1px solid #e5e7eb; margin: 15px 0;">
            <p><strong>Seats:</strong> ${booking.seatNumbers.join(', ')}</p>
            <p><strong>Total Refund Reference:</strong> ₹${booking.totalPrice}</p>
            <p><strong>Booking ID:</strong> ${booking._id}</p>
            <p><strong>Status:</strong> ${booking.status}</p>
          </div>

          <p>If you did not request this cancellation or believe this is a mistake, please contact support.</p>
          <p style="color: #6b7280; font-size: 12px;">SkyBook Team</p>
        </div>
      `,
    });
